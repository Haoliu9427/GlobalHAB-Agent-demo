...
- **现场影像甄别**：手机拍摄或上传海面照片，进行图像质量检查、视觉异常初筛和复核优先级判断；默认是透明的规则/颜色纹理演示基线，不冒充已训练藻华分类器；
...
默认实现是 `规则 + 通用颜色/纹理特征演示基线`，不是已经训练完成的HAB图片分类器。页面输出的是 `低 / 中 / 高 / DEFER` **复核优先级**，不是HAB发生概率；普通照片不能确诊具体藻种或毒素。完成甄别后可一键跳转到“大模型结果解读”，但只传递结构化文字摘要，照片本身不会自动发送给远程模型。完整说明见 `FIELD_VISUAL_SCREENING_GUIDE.md`。
