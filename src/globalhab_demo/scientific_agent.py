...
from .planning_models import MODEL_NAMES, inventory, fit_predict
...
ACTIONS = {a.action_id: a for a in (
    ExperimentAction(*x) for x in product(
        ("local", "downstream"), LAGS, tuple(MODEL_NAMES)
    )
)}
...
先根据模型目录、数据条件与预算，用select_models选2至3种模型并说明理由，然后检验至少两种不同模型。
看到反馈后可以再次select_models补充2至3种模型；未选模型不能运行。
工具：select_models {"model_ids":["目录中的ID","另一个ID"]}；inspect_data {}；evaluate_hypothesis {"action_id":"目录中的ID"}；
...
class ScientificTools:
    ...
    def _fit(self, train, target, name, features=None):
        ...
        if features is not None:
            ...
            p = fit_predict(name, train, target, self.seed)
    ...
    def describe(self):
        return {
            ...
            "selected_models": sorted(self.selected_models),
            "catalog": [aid for aid,a in ACTIONS.items() if a.model in self.selected_models],
    ...
    def execute(self, tool, args):
        ...
        if tool == "select_models":
            ids = args.get("model_ids")
            allowed = {r["model_id"] for r in inventory() if r["available"]}
            if set(args) != {"model_ids"} or not isinstance(ids,list) or not all(isinstance(i,str) for i in ids) or not 2 <= len(set(ids)) <= 3 or any(i not in allowed for i in ids):
                raise ValueError("须从当前可运行目录选择2至3种不同模型")
            self.selected_models.update(ids)
        ...
        aid = args["action_id"]
        if tool == "finish":
            if len({ACTIONS[k].model for k in self.candidates}) < 2:
                raise ValueError("冻结前须实际比较至少两种不同预测模型")
        ...
        if ACTIONS[aid].model not in self.selected_models:
            raise ValueError("先通过select_models选择该预测模型")
        kind = None
...
def run_scientific_agent(frame, planner, *, goal="比较本地和上游环境关联，寻找可复核的风险线索", budget=8, seed=42,
    ...
    for step in range(1, budget+6):
        ...
        if consecutive_rejections >= 3:
            status = "invalid_planning_stopped"
            break
    ...
    return {"version": "HF3.9", "controller": "llm_json_tool_planner", "model": model_label, "status": status,
