    ...
    with st.container(border=True, key="research_section_llm_comparison"):
        ...
        with left:
            if not comparison.empty:
                research_plot(baseline_comparison(comparison), width="stretch", config={"displayModeBar":False}, key="llm_final_comparison")
                st.caption("冻结方案后的独立测试；AP 为平均精确率，越高表示风险排序越好。")
        ...
        with right:
            ...
            if candidates:
                ...
                aid = st.selectbox("查看候选的对照证据",ids,index=ids.index(frozen) if frozen in ids else 0,format_func=candidate_label)
                research_plot(control_figure(result,aid),width="stretch",config={"displayModeBar":False},key="llm_control_comparison")
                ...
                st.caption("尚未执行："+"、".join(missing) if missing else "两种负对照均已执行。单次对照是诊断，不代表因果证明或显著性检验。")
            else:
