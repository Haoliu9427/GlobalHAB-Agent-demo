...
def render(manual=False):
    st.markdown("### " + ("自选模型 · 连接服务" if manual else "平台模型 · 智能研究"))
    ...
    with st.container(border=True):
        ...
        if manual:
            ...
            st.caption("使用你自己的服务配置和额度。平台模型的密钥不会填入此处。")
        else:
            entries = catalog()
            if entries:
                ...
                selected = st.selectbox("平台提供的模型", list(by_id), format_func=lambda k: by_id[k]["label"], key="science_platform_model")
